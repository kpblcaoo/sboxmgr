...
from .common import TlsConfig, TransportConfig, MultiplexConfig
...
class OutboundBase(SingBoxModelBase):
    ...
    server_port: Optional[int] = Field(default=None, ge=1, le=65535, description="Server port.")
    tls: Optional[TlsConfig] = Field(default=None, description="TLS settings for secure connection.")
    multiplex: Optional[MultiplexConfig] = Field(default=None, description="Multiplexing settings.")
    local_address: Optional[List[str]] = Field(default=None, description="Local addresses to bind.")
    domain_strategy: Optional[DomainStrategy] = Field(default=None, description="Domain resolution strategy.")
...
class ShadowsocksOutbound(OutboundWithTransport):
    ...
    plugin: Optional[str] = Field(default=None, description="Obfuscation plugin, e.g., 'v2ray-plugin'.")
    plugin_opts: Optional[Dict[str, Any]] = Field(default=None, description="Plugin options, e.g., {'mode': 'websocket'}.")
    udp_over_tcp: Optional[bool] = Field(default=None, description="Enable UDP over TCP.")
...
class VmessOutbound(OutboundWithTransport):
    ...
    alter_id: Optional[int] = Field(default=0, description="Alternative ID count (0-65535).")
    global_padding: Optional[bool] = Field(default=None, description="Enable global padding for obfuscation.")
    authenticated_length: Optional[bool] = Field(default=None, description="Enable authenticated length for security.")
    packet_encoding: Optional[Literal["packet", "xudp"]] = Field(default=None, description="Packet encoding method.")
...
class WireGuardOutbound(OutboundBase):
    ...
    pre_shared_key: Optional[str] = Field(default=None, description="Pre-shared key for authentication.")
    mtu: Optional[int] = Field(default=0, ge=0, description="Maximum transmission unit (0 for auto).")
    keepalive: Optional[str] = Field(default=None, description="Persistent keepalive interval, e.g., '25s'.")
    ...
    reserved: Optional[List[int]] = Field(default=None, description="Reserved bits for compatibility.")
    local_address: Optional[List[str]] = Field(default=None, description="Local addresses to bind.")
...
class ShadowTlsOutbound(OutboundWithTransport):
    ...
    handshake_for_internal: Optional[Dict[str, Any]] = Field(default=None, description="Internal handshake settings.")
...
class HysteriaOutbound(OutboundBase):
    ...
    recv_window_conn: Optional[int] = Field(None, ge=0, description="QUIC stream-level flow control window.")
    recv_window: Optional[int] = Field(None, ge=0, description="QUIC connection-level flow control window.")
    disable_mtu_discovery: Optional[bool] = Field(None, description="Disable Path MTU Discovery.")
...
class AnyTlsOutbound(OutboundBase):
    ...
    idle_session_timeout: Optional[str] = Field(None, description="Timeout for idle sessions.")
    min_idle_session: Optional[int] = Field(None, ge=0, description="Minimum idle sessions to keep.")
...
class SshOutbound(OutboundBase):
    ...
    host_key_algorithms: Optional[List[str]] = Field(None, description="Host key algorithms.")
...
class TorOutbound(OutboundBase):
    ...
    torrc: Optional[Dict[str, Any]] = Field(None, description="Tor configuration options.")
