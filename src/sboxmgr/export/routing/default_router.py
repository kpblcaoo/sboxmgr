...
class DefaultRouter(BaseRoutingPlugin):
    ...
    def generate_routes(
        ...
        if exclusions:
            ...
            if excluded_cidrs:
                rules.append({"ip_cidr": excluded_cidrs, "outbound": "direct"})

            # Add rules for excluded domains
            if excluded_domains:
                rules.append({"domain": excluded_domains, "outbound": "direct"})
        ...
        if servers and len(servers) > 0:
            ...
            for server in servers:
                ...
                if not server_tag:
                    server_tag = (
                        f"{server.type}-{server.address}"
                        if hasattr(server, "type") and hasattr(server, "address")
                        else None
                    )
            ...
            if selected_server:
                ...
                if hasattr(selected_server, "meta") and selected_server.meta:
                    server_tag = selected_server.meta.get(
                        "tag"
                    ) or selected_server.meta.get("name", "")
                ...
                if not server_tag:
                    server_tag = (
                        f"{selected_server.type}-{selected_server.address}"
                        if hasattr(selected_server, "type")
                        and hasattr(selected_server, "address")
                        else "direct"
                    )

                if debug_level >= 1:
                    print(
                        f"[DefaultRouter] Selected server for default route: {server_tag}"
                    )

                # Add default proxy rule - route all other traffic through selected server
                rules.append({"outbound": server_tag})
            else:
