...
def load_exclusions(dry_run=False):
    ...
    if file_exists(EXCLUSION_FILE):
        try:
            ...
            if not dry_run:
                logging.error(
                    f"Файл {EXCLUSION_FILE} повреждён или невалиден. Игнорируем содержимое и продолжаем с пустым списком исключений."
                )
                typer.echo(
                    "[Ошибка] exclusions.json повреждён или невалиден. Сброшен до пустого состояния."
                )
    ...
    for item in exclude_list:
        if item.startswith("-") and item[1:].isdigit():
            ...
            if 0 <= index < len(supported_servers):
                ...
                server_id = (
                    server.get("id", None)
                    or f"{server.get('tag', '')}{server.get('type', '')}{server.get('server_port', '')}"
                )
                new_exclusions = [ex for ex in new_exclusions if ex["id"] != server_id]
                if debug_level >= 0:
                    logging.info(
                        f"Removed exclusion for server at index {index}: {server.get('tag', 'N/A')}"
                    )
                    typer.echo(
                        f"Removed exclusion for server at index {index}: {server.get('tag', 'N/A')}"
                    )
...
def view_exclusions(debug_level=0):
    ...
    for exclusion in exclusions["exclusions"]:
        typer.echo(
            f"ID: {exclusion['id']}, Name: {exclusion['name']}, Reason: {exclusion.get('reason', 'N/A')}"
        )
    if debug_level >= 2:
