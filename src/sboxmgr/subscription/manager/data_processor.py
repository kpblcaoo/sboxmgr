...
class DataProcessor:
    ...
    def fetch_and_validate_raw(self, context: PipelineContext) -> tuple[bytes, bool]:
        ...
        try:
            ...
            err = self.error_handler.create_fetch_error(
                str(e), {"source_type": self.fetcher.source.source_type}
            )
    ...
    def validate_parsed_servers(
        ...
        try:
            ...
            err = self.error_handler.create_validation_error(
                "parsed_validate", str(e), {"server_count": len(servers)}
            )
            self.error_handler.add_error_to_context(context, err)
    ...
    def _log_parse_result(
        ...
        if debug_level >= 1:
            print(
                f"[debug] Parser {parser.__class__.__name__} returned {len(servers)} servers"
            )
    ...
    def _handle_validation_results(
        ...
        for error in parsed_result.errors:
            err = self.error_handler.create_validation_error(
                "parsed_validate", error, {"total_servers": len(servers)}
            )
            self.error_handler.add_error_to_context(context, err)
