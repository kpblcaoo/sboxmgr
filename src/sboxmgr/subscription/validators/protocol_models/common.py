...
class TlsConfig(BaseModel):
    ...
    min_version: Optional[Literal["1.0", "1.1", "1.2", "1.3"]] = Field(
        None, description="Minimum TLS version"
    )
    max_version: Optional[Literal["1.0", "1.1", "1.2", "1.3"]] = Field(
        None, description="Maximum TLS version"
    )
    ...
    certificate: Optional[str] = Field(
        None, description="Certificate content in PEM format"
    )
    ...
    utls: Optional[UtlsConfig] = Field(
        None, description="uTLS settings for TLS client emulation"
    )
    reality: Optional[RealityConfig] = Field(
        None, description="Reality settings for SNI protection"
    )
...
class StreamSettings(BaseModel):
    ...
    network: Optional[Literal["tcp", "udp", "ws", "http", "grpc", "quic"]] = Field(
        None, description="Transport type (TCP, WebSocket, gRPC, etc.)"
    )
    security: Optional[Literal["none", "tls", "reality"]] = Field(
        None, description="Encryption type (TLS, REALITY or none)"
    )
    tls_settings: Optional[TlsConfig] = Field(
        None, description="TLS settings for transport"
    )
...
class MultiplexConfig(BaseModel):
    ...
    protocol: Optional[Literal["smux", "yamux", "h2mux"]] = Field(
        None, description="Multiplexing protocol"
    )
    max_connections: Optional[int] = Field(
        None, ge=1, description="Maximum number of connections"
    )
    min_streams: Optional[int] = Field(
        None, ge=0, description="Minimum number of streams"
    )
    max_streams: Optional[int] = Field(
        None, ge=0, description="Maximum number of streams"
    )
