...
class CustomEnricher:
        ...
        if hasattr(profile, 'subscriptions') and profile.subscriptions:
            for sub_profile in profile.subscriptions:
                if context.source and sub_profile.id in context.source:
                    ...
                    server.meta['subscription_priority'] = getattr(sub_profile, 'priority', 1)
                    server.meta['subscription_enabled'] = getattr(sub_profile, 'enabled', True)
    ...
    def _apply_priority_scoring(
        ...
        if hasattr(profile, 'filters') and profile.filters:
            # Preferred tags
            if hasattr(profile.filters, 'only_tags') and profile.filters.only_tags and server.tag:
                if server.tag in profile.filters.only_tags:
                    priority_score += 0.3

            # Excluded tags (negative scoring)
            if hasattr(profile.filters, 'exclude_tags') and profile.filters.exclude_tags and server.tag:
        ...
        if hasattr(profile, 'routing') and profile.routing:
            # Preferred countries
            if hasattr(profile.routing, 'preferred_countries') and profile.routing.preferred_countries:
                server_country = server.meta.get('geo', {}).get('country')
                if server_country and server_country in profile.routing.preferred_countries:
                    priority_score += 0.2
    ...
    def _apply_compatibility_check(
        ...
        if export_format == 'sing-box':
            ...
            supported_protocols = {
                ...
                'hysteria2', 'tuic', 'shadowtls', 'ssh', 'tor'
            }
            ...
            supported_protocols = {
                'shadowsocks', 'vmess', 'trojan', 'http', 'socks', 'socks5'
            }
            if server.type not in supported_protocols:
                compatibility['compatible'] = False
            ...
            supported_protocols = {
                'shadowsocks', 'vmess', 'vless', 'trojan', 'http', 'socks'
            }
            if server.type not in supported_protocols:
                compatibility['compatible'] = False
                compatibility['issues'].append(f'Protocol {server.type} not supported by v2ray')
