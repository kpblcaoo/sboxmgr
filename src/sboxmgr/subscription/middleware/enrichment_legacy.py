...
class EnrichmentMiddleware(TransformMiddleware):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.enable_performance_enrichment = self.config.get('enable_performance_enrichment', True)
        self.enable_security_enrichment = self.config.get('enable_security_enrichment', True)
        self.enable_custom_enrichment = self.config.get('enable_custom_enrichment', True)
        self.geo_database_path = self.config.get('geo_database_path')
        self.performance_cache_duration = self.config.get('performance_cache_duration', 300)
    ...
    def _do_process(
        ...
        for server in servers:
            ...
            try:
                ...
                enriched_server = self._enrich_server(server, context, profile, enrichment_config)
    ...
    def _extract_enrichment_config(self, profile: Optional[FullProfile]) -> Dict[str, Any]:
        """Extract enrichment configuration from profile.
    ...
    def _enrich_server(
        ...
        if enrichment_config['enable_custom_enrichment']:
            server = self._apply_custom_enrichment(server, context, profile, enrichment_config)

        return server

    def _apply_basic_enrichment(self, server: ParsedServer, context: PipelineContext) -> ParsedServer:
        """Apply basic metadata enrichment.
    ...
    def _apply_geo_enrichment(self, server: ParsedServer, context: PipelineContext) -> ParsedServer:
        """Apply geographic enrichment.
    ...
    def _apply_performance_enrichment(self, server: ParsedServer, context: PipelineContext) -> ParsedServer:
    ...
    def _estimate_latency_class(self, server: ParsedServer) -> str:
        ...
        if 'geo' in server.meta:
            ...
            if geo_info.get('country') in ['US', 'CA', 'GB', 'DE', 'FR', 'JP', 'KR', 'SG']:
                return 'low'
    ...
    def _apply_security_enrichment(self, server: ParsedServer, context: PipelineContext) -> ParsedServer:
    ...
    def _apply_subscription_tags(self, server: ParsedServer, context: PipelineContext, profile: FullProfile) -> None:
    ...
    def _apply_priority_scoring(self, server: ParsedServer, context: PipelineContext, profile: FullProfile) -> None:
    ...
    def _apply_compatibility_check(self, server: ParsedServer, context: PipelineContext, profile: FullProfile) -> None:
        ...
        if export_format == 'sing-box':
            ...
            if server.type not in ['shadowsocks', 'vmess', 'vless', 'trojan', 'wireguard']:
                compatibility['compatible'] = False
                compatibility['issues'].append(f'Protocol {server.type} not supported by sing-box')
