...
from .chain_builders import (
    create_middleware_chain_from_list,
    create_postprocessor_chain_from_list,
)
...
def export(
    ...
    export_format: str = typer.Option(
        "singbox", "--export-format", help="Export format: singbox, clash"
    ),
    ...
    client_profile: str = typer.Option(
        None,
        "--client-profile",
        help="Client profile JSON file for inbound configuration",
    ),
    ...
    generate_profile: str = typer.Option(
        None,
        "--generate-profile",
        help="Generate profile JSON file from CLI parameters and exit",
    ),
    # Inbound CLI parameters
    inbound_types: str = typer.Option(
        None,
        "--inbound-types",
        help="Comma-separated inbound types (tun,socks,http,tproxy)",
    ),
    # TUN parameters
    tun_address: str = typer.Option(
        None, "--tun-address", help="TUN interface address (default: 198.18.0.1/16)"
    ),
    tun_mtu: int = typer.Option(
        None, "--tun-mtu", help="TUN MTU value (default: 1500)"
    ),
    tun_stack: str = typer.Option(
        None,
        "--tun-stack",
        help="TUN network stack (system,gvisor,mixed, default: mixed)",
    ),
    # SOCKS parameters
    socks_port: int = typer.Option(
        None, "--socks-port", help="SOCKS proxy port (default: 1080)"
    ),
    socks_listen: str = typer.Option(
        None, "--socks-listen", help="SOCKS bind address (default: 127.0.0.1)"
    ),
    socks_auth: str = typer.Option(
        None, "--socks-auth", help="SOCKS authentication (user:pass)"
    ),
    # HTTP parameters
    http_port: int = typer.Option(
        None, "--http-port", help="HTTP proxy port (default: 8080)"
    ),
    http_listen: str = typer.Option(
        None, "--http-listen", help="HTTP bind address (default: 127.0.0.1)"
    ),
    http_auth: str = typer.Option(
        None, "--http-auth", help="HTTP authentication (user:pass)"
    ),
    # TPROXY parameters
    tproxy_port: int = typer.Option(
        None, "--tproxy-port", help="TPROXY port (default: 7895)"
    ),
    tproxy_listen: str = typer.Option(
        None, "--tproxy-listen", help="TPROXY bind address (default: 0.0.0.0)"
    ),
    # DNS mode
    dns_mode: str = typer.Option(
        None,
        "--dns-mode",
        help="DNS resolution mode (system,tunnel,off, default: system)",
    ),
    # Routing and filtering flags
    final_route: str = typer.Option(
        None,
        "--final-route",
        help="Set final routing destination (e.g., proxy, direct, block)",
    ),
    exclude_outbounds: str = typer.Option(
        None,
        "--exclude-outbounds",
        help="Comma-separated list of outbound types to exclude (e.g., direct,block,dns)",
    ),
):
    ...
    if final_route or exclude_outbounds:
        ...
        if exclude_outbounds:
            exclude_list = [
                o.strip() for o in exclude_outbounds.split(",") if o.strip()
            ]
