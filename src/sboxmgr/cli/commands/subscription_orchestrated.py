...
def run_orchestrated(
    ...
    config_file: str = typer.Option(
        None, "--config-file", help=t("cli.config_file.help")
    ),
    backup_file: str = typer.Option(
        None, "--backup-file", help=t("cli.backup_file.help")
    ),
    user_agent: str = typer.Option(
        None, "--user-agent", help="Custom User-Agent header"
    ),
    no_user_agent: bool = typer.Option(
        False, "--no-user-agent", help="Disable User-Agent"
    ),
    format: str = typer.Option("singbox", "--format", help="Export format"),
    user_routes: str = typer.Option(
        None, "--user-routes", help="Comma-separated list of route tags to include"
    ),
    exclusions: str = typer.Option(
        None, "--exclusions", help="Comma-separated list of servers to exclude"
    ),
):
    ...
    user_routes_list = (
        [x.strip() for x in user_routes.split(",")] if user_routes else None
    )
    ...
    try:
        ...
        if not export_result["success"]:
            typer.echo(
                f"ERROR: {export_result.get('error', 'Export failed')}", err=True
            )
            raise typer.Exit(1)
    ...
    try:
        ...
        if debug >= 1:
            typer.echo(
                "Configuration generated successfully. Use sboxagent to apply and restart service."
            )
...
def exclusions_orchestrated(
    ...
    try:
        result = orchestrator.manage_exclusions(
            action=action, server_id=server_id, name=name, reason=reason
        )
...
def dry_run_orchestrated(
    ...
    format: str = typer.Option(
        "singbox", "--format", help="Export format: singbox, clash, v2ray"
    ),
    user_routes: str = typer.Option(
        None, "--user-routes", help="Comma-separated list of route tags to include"
    ),
...
def list_servers_orchestrated(
    ...
    no_user_agent: bool = typer.Option(
        False, "--no-user-agent", help="Disable User-Agent header"
    ),
    format_output: str = typer.Option(
        "table", "--format", help="Output format: table, json, yaml"
    ),
    show_details: bool = typer.Option(
        False, "--details", help="Show detailed server information"
    ),
    filter_protocol: str = typer.Option(
        None, "--protocol", help="Filter by protocol (vmess, vless, trojan, etc.)"
    ),
    user_routes: str = typer.Option(
        None, "--user-routes", help="Comma-separated route tags to include"
    ),
    exclusions: str = typer.Option(
        None, "--exclusions", help="Comma-separated servers to exclude"
    ),
):
        ...
        if format_output == "json":
            ...
            for i, server in enumerate(servers, 1):
                ...
                if show_details:
                    ...
                    typer.echo(
                        f"{i:3d}. [{protocol:8s}] {server_name:30s} {server_addr}:{server_port}"
                    )
