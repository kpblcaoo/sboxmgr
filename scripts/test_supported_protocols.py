...
def test_protocol_support():
    ...
    outbound_protocols = [
        "vless", "vmess", "shadowsocks", "trojan", "direct", "block",
        "http", "socks", "dns", "selector", "urltest", "wireguard",
        "hysteria", "hysteria2", "tuic", "ssh", "tor", "shadowtls", "anytls"
    ]
    
    inbound_protocols = [
        "http", "socks", "mixed", "tun", "redirect", "tproxy",
        "shadowsocks", "vmess", "vless", "trojan", "hysteria", "hysteria2",
        "tuic", "naive", "shadowtls", "anytls", "direct"
    ]
    ...
    for protocol in outbound_protocols:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "unknown" in result.stderr.lower() or "unsupported" in result.stderr.lower():
    ...
    # Summary
    print(f"\n📊 SUMMARY:")
    print(f"  Supported outbounds: {len(supported_outbounds)}/{len(outbound_protocols)}")
    print(f"  Supported inbounds: {len(supported_inbounds)}/{len(inbound_protocols)}")
    print(f"  Total supported: {len(supported_outbounds) + len(supported_inbounds)}/{len(outbound_protocols) + len(inbound_protocols)}")
