...
## Next steps
- [ ] Перенос реальной логики prepare_selection и load_outbounds в cli_common.py.
- [ ] Расширение matrix-тестов по мере добавления новых CLI-флагов.
- [ ] Поддерживать changelog и таблицу CLI-сценариев в актуальном состоянии.
- [ ] Проверить, что новые функции не вызывают циклических импортов.
- [ ] Внедрение Typer для CLI: декомпозиция main.py на команды, автогенерация help, тесты через Typer.CliRunner.
- [ ] Документация и changelog по новой архитектуре CLI.
- [x] Массовое переименование: импорты, env, help, systemd, install/wizard, тесты — sboxmgr/sboxctl, кроме SINGBOX_URL и переменных для оригинального sing-box.
