...
def test_list_servers_deprecated_alias_works(monkeypatch):
    """Тестирует, что deprecated команда list-servers работает и показывает предупреждение."""
    runner = CliRunner()
    ...
    # Проверяем наличие предупреждения об устаревании
    assert "Warning: 'list-servers' is deprecated" in output
