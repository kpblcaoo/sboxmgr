...
def mock_all_fetchers_and_parser():
    ...
    # Mock fetchers
    with (
        patch("sboxmgr.subscription.fetchers.apifetcher.ApiFetcher.fetch") as mock_api,
        patch(
            "sboxmgr.subscription.fetchers.file_fetcher.FileFetcher.fetch"
        ) as mock_file,
        patch(
...
class TestCLIRoutingFlags:
    ...
    @pytest.mark.skip(
        reason="TODO: CLI tests require deeper analysis of architecture and mocking strategy. Will be addressed in separate branch."
    )
    def test_final_route_flag(self):
        """Test --final-route flag functionality."""
        result = self.runner.invoke(
            export_app, ["--url", "https://example.com", "--final-route", "direct"]
        )
    ...
    def test_exclude_outbounds_flag(self):
        ...
        result = self.runner.invoke(
            export_app,
            ["--url", "https://example.com", "--exclude-outbounds", "vmess,ss"],
        )
        assert result.exit_code == 0
        assert "Applied routing" in result.stdout or "✅" in result.stdout
    ...
    def test_invalid_final_route(self):
        ...
        result = self.runner.invoke(
            export_app, ["--url", "https://example.com", "--final-route", "invalid"]
        )
        assert result.exit_code != 0
    ...
    def test_empty_exclude_outbounds(self):
        ...
        result = self.runner.invoke(
            export_app, ["--url", "https://example.com", "--exclude-outbounds", ""]
        )
        assert result.exit_code == 0
        assert "Applied routing" in result.stdout or "✅" in result.stdout
