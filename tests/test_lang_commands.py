...
from sboxmgr.cli.commands.lang import LANG_NAMES, lang_cmd
...
class TestLangCmd:
    ...
    def test_lang_cmd_set_language_success(self, tmp_path):
        ...
        with patch("sboxmgr.cli.commands.lang.Path.home", return_value=tmp_path), patch(
            ...
            mock_echo.assert_called_with(
                f"Language set to 'ru' and persisted in {config_file}."
            )
    ...
    def test_lang_cmd_set_language_config_write_error(self, tmp_path):
        ...
        with patch("sboxmgr.cli.commands.lang.Path.home", return_value=tmp_path), patch(
            ...
            mock_echo.assert_any_call(
                "[Error] Failed to write config: Access denied", err=True
            )
...
class TestLangCmdEdgeCases:
    ...
    def test_lang_cmd_usage_instructions(self):
        ...
        with patch(
            ...
            mock_echo.assert_any_call(
                "To set language persistently: sboxctl lang --set ru"
            )
            mock_echo.assert_any_call(
                "Or for one-time use: SBOXMGR_LANG=ru sboxctl ..."
            )
