...
class TestValidateProtocol:
    ...
    def test_validate_protocol_unsupported_protocol(self):
        ...
        with patch('sboxmgr.config.protocol.error') as mock_error:
            with pytest.raises(ValueError, match="Unsupported protocol: unsupported_protocol"):
    ...
    def test_validate_protocol_vless_missing_required_server(self):
        ...
        with patch('sboxmgr.config.protocol.error') as mock_error:
            with pytest.raises(ValueError, match="Missing required parameter for vless: server"):
                validate_protocol(config, supported_protocols)
        
        mock_error.assert_called_once_with("Missing required parameter for vless: server")
    ...
    def test_validate_protocol_vless_missing_required_uuid(self):
        ...
        with patch('sboxmgr.config.protocol.error') as mock_error:
            with pytest.raises(ValueError, match="Missing required parameter for vless: uuid"):
                validate_protocol(config, supported_protocols)
    ...
    def test_validate_protocol_shadowsocks_missing_method(self):
        ...
        with patch('sboxmgr.config.protocol.error') as mock_error:
            with pytest.raises(ValueError, match="Missing required parameter for shadowsocks: method"):
                validate_protocol(config, supported_protocols)
        
        mock_error.assert_called_once_with("Missing required parameter for shadowsocks: method")
    ...
    def test_validate_protocol_trojan_missing_password(self):
        ...
        with patch('sboxmgr.config.protocol.error') as mock_error:
            with pytest.raises(ValueError, match="Missing required parameter for trojan: password"):
                validate_protocol(config, supported_protocols)
        
        mock_error.assert_called_once_with("Missing required parameter for trojan: password")
    ...
    def test_validate_protocol_hysteria2_missing_password(self):
        ...
        with patch('sboxmgr.config.protocol.error') as mock_error:
            with pytest.raises(ValueError, match="Missing required parameter for hysteria2: password"):
                validate_protocol(config, supported_protocols)
        
        mock_error.assert_called_once_with("Missing required parameter for hysteria2: password")
    ...
    def test_validate_protocol_no_handler_defined(self):
        ...
        supported_protocols = ["unknown_protocol"]  # Protocol is supported but no handler
        
        with patch('sboxmgr.config.protocol.error') as mock_error:
            with pytest.raises(ValueError, match="No handler defined for protocol: unknown_protocol"):
                validate_protocol(config, supported_protocols)
        
        mock_error.assert_called_once_with("No handler defined for protocol: unknown_protocol")
...
class TestValidateProtocolEdgeCases:
    ...
    def test_validate_protocol_zero_port(self):
        ...
        with patch('sboxmgr.config.protocol.error') as mock_error:
            with pytest.raises(ValueError, match="Missing required parameter for vless: server_port"):
                validate_protocol(config, supported_protocols)
        
        mock_error.assert_called_once_with("Missing required parameter for vless: server_port")
