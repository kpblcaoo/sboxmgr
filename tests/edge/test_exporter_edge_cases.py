...
def test_invalid_server_fields():
    ...
    edge_case_servers = [
        ...
        ParsedServer(
            type="vmess", address="valid.com", port=443
        ),  # Валидный для сравнения
    ]
...
def test_duplicate_servers():
    server = ParsedServer(
        type="vmess", address="1.2.3.4", port=443, meta={"tag": "dup"}
    )
...
def test_too_long_values():
    long_str = "A" * 1000
    servers = [
        ParsedServer(type="vmess", address=long_str, port=443, meta={"tag": long_str})
    ]
    config = singbox_export(servers, routes=[])
    assert isinstance(config, dict)
    assert "outbounds" in config
...
def test_failtolerance_skip_unsupported_type(caplog):
    ...
    servers = [
        ...
        ParsedServer(
            ...
            address="ok2.com",
            port=8388,
            meta={"method": "aes-256-gcm", "password": "p"},
        ),
    ]
    config = singbox_export(servers, routes=[])
    tags = [o["tag"] for o in config["outbounds"]]
...
def test_export_hysteria2_success(caplog):
    servers = [
        ParsedServer(
            type="hysteria2", address="h.com", port=443, password="pw", tag="hyst2"
        )
    ]
    config = singbox_export(servers, routes=[])
...
def test_export_anytls_success(caplog):
    servers = [
        ParsedServer(
            type="anytls", address="a.com", port=443, uuid="uuid", tag="anytls"
        )
    ]
    config = singbox_export(servers, routes=[])
...
def test_inbounds_valid_profile():
    ...
    profile = ClientProfile(
        inbounds=[
            ...
            InboundProfile(
                type="tun", listen="127.0.0.1", port=12345, options={"stack": "system"}
            ),
        ]
    )
...
def test_exporter_wireguard_missing_fields():
    ...
    assert "outbounds" in config
    assert all(
        o.get("type") != "wireguard" or o.get("address") for o in config["outbounds"]
    )
...
def test_exporter_tuic_invalid():
    ...
    assert "outbounds" in config
    assert all(
        o.get("type") != "tuic" or o.get("port", 0) > 0 for o in config["outbounds"]
    )
...
def test_exporter_hysteria_missing_fields():
    ...
    assert "outbounds" in config
    assert all(
        o.get("type") != "hysteria" or o.get("address") for o in config["outbounds"]
    )
