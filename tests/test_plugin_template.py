...
def test_plugin_template_generates_all_types(tmp_path):
    ...
    for t in ALL_TYPES:
        ...
        result = runner.invoke(
            plugin_template.app, [t, name, "--output-dir", str(output_dir)]
        )
...
def test_plugin_template_invalid_type(tmp_path):
    ...
    result = runner.invoke(
        plugin_template.app,
        ["invalidtype", "TestInvalid", "--output-dir", str(output_dir)],
    )
...
def test_plugin_template_output_dir_error(monkeypatch, tmp_path):
    ...
    result = runner.invoke(
        plugin_template.app,
        ["fetcher", "TestFetcher", "--output-dir", str(tmp_path / "fail_dir")],
    )
    assert result.exit_code != 0
...
class TestPluginTemplate:
    ...
    def test_plugin_template_fetcher(self, tmp_path):
        ...
        assert (
            "from sboxmgr.subscription.fetchers.customfetcher import CustomFetcher"
            in test_content
        )
    ...
    def test_plugin_template_parsed_validator(self, tmp_path):
        ...
        plugin_template_func(
            type="parsed_validator", name="Custom", output_dir=output_dir
        )
    ...
    def test_plugin_template_postprocessor(self, tmp_path):
        ...
        assert (
            "def process(self, servers: list[ParsedServer], context) -> list[ParsedServer]:"
            in content
        )
    ...
    def test_plugin_template_invalid_type(self, tmp_path):
        ...
        with patch("typer.echo") as mock_echo:
            with pytest.raises(typer.Exit) as exc_info:
                plugin_template_func(
                    type="invalid", name="Custom", output_dir=output_dir
                )
    ...
    def test_plugin_template_name_already_has_suffix(self, tmp_path):
        ...
        output_dir = str(tmp_path / "test_output")
        plugin_template_func(
            type="fetcher", name="CustomFetcher", output_dir=output_dir
        )
    ...
    def test_plugin_template_default_output_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                os.chdir(tmp_dir)
                plugin_template_func(
                    type="fetcher", name="Custom", output_dir="plugin_templates"
                )
    ...
    def test_plugin_template_file_write_error(self, tmp_path):
        ...
        with patch("builtins.open", side_effect=OSError("Disk full")), patch(
            ...
            mock_echo.assert_any_call(
                "[ERROR] Failed to write template files: Disk full", err=True
            )
...
class TestPluginTemplateContent:
    ...
    def test_fetcher_template_content(self, tmp_path):
        ...
        # Check docstring
        assert (
            "MyCustomFetcher fetches subscription data from custom source." in content
        )
    ...
    def test_test_template_content(self, tmp_path):
        ...
        assert (
            "from sboxmgr.subscription.fetchers.mycustomfetcher import MyCustomFetcher"
            in content
        )
...
class TestPluginTemplateEdgeCases:
    ...
    def test_plugin_template_all_types_coverage(self, tmp_path):
        ...
        types_and_expected = [
            ...
            (
                "validator",
                "BaseValidator",
                "def validate(self, raw: bytes, context=None):",
            ),
            (
                "parsed_validator",
                "BaseParsedValidator",
                "def validate(self, servers: list[ParsedServer], context):",
            ),
    ...
    def test_plugin_template_debug_output(self, tmp_path):
        ...
        mock_echo.assert_any_call(
            f"[DEBUG] Attempting to write template to {plugin_file}"
        )
