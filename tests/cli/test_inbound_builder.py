...
class TestInboundBuilder:
    ...
    def test_add_tun_custom_params(self):
        ...
        profile = builder.add_tun(
            address="10.0.0.1/24", mtu=1400, stack="system", auto_route=False
        ).build()
    ...
    def test_add_socks_invalid_auth(self):
        ...
        builder = InboundBuilder()
        with pytest.raises(
            ValueError, match="Auth must be in format 'username:password'"
        ):
...
class TestBuildClientProfileFromCli:
    ...
    def test_single_tun_inbound(self):
        ...
        profile = build_client_profile_from_cli(
            inbound_types="tun", tun_address="10.0.0.1/24", tun_mtu=1400
        )
    ...
    def test_multiple_inbound_types(self):
        ...
        profile = build_client_profile_from_cli(
            inbound_types="tun,socks,http", socks_port=1080, http_port=8080
        )
    ...
    def test_socks_with_auth(self):
        """Test SOCKS inbound with authentication."""
        profile = build_client_profile_from_cli(
            inbound_types="socks", socks_auth="user:pass"
        )
    ...
    def test_custom_dns_mode(self):
        ...
        profile = build_client_profile_from_cli(inbound_types="tun", dns_mode="tunnel")
    ...
    def test_whitespace_handling(self):
        ...
        profile = build_client_profile_from_cli(inbound_types=" tun , socks , http ")
