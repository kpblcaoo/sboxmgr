...
class TestProfileModels:
    ...
    def test_full_profile_with_all_components(self):
        ...
        export = ExportProfile(format=ExportFormat.CLASH, output_file="clash.yaml")

        agent = AgentProfile(
            auto_restart=True, monitor_latency=False, health_check_interval="60s"
        )

        ui = UIProfile(default_language="ru", mode=UIMode.TUI, show_debug_info=True)
    ...
    def test_profile_json_serialization(self):
        ...
        original = FullProfile(
            ...
            subscriptions=[SubscriptionProfile(id="sub1", enabled=True, priority=1)],
        )
