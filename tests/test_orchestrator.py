...
class TestOrchestratorError:
    ...
    def test_error_with_context(self):
        ...
        error = OrchestratorError(
            "Orchestrator error", operation="test_op", cause=cause
        )
...
class TestOrchestrator:
    ...
    def test_lazy_loading_exclusion_manager(self):
        ...
        with patch(
            "sboxmgr.core.factory.create_default_exclusion_manager"
        ) as mock_factory:
            mock_manager = Mock()
    ...
    def test_manage_exclusions_basic(self):
        ...
        result = orchestrator.manage_exclusions(action="add", server_id="test-server")
    ...
    def test_get_subscription_servers_success(self):
        ...
        with patch(
            "sboxmgr.subscription.manager.SubscriptionManager",
            return_value=mock_sub_mgr,
        ):
    ...
    def test_manage_exclusions_add(self):
        ...
        result = orchestrator.manage_exclusions(
            action="add", server_id="test-server", name="Test Server", reason="Testing"
        )
    ...
    def test_manage_exclusions_remove(self):
        ...
        result = orchestrator.manage_exclusions(
            action="remove", server_id="test-server"
        )
    ...
    def test_export_configuration_success(self):
        ...
        mock_result = PipelineResult(
            config=[Mock()], context=PipelineContext(), errors=[], success=True
        )

        # Mock the get_subscription_servers method directly
        with patch.object(
            orchestrator, "get_subscription_servers", return_value=mock_result
        ):
            # Mock successful export
            orchestrator._export_manager.export.return_value = {"test": "config"}

            result = orchestrator.export_configuration(
                source_url="https://example.com/sub", export_format="singbox"
            )
    ...
    def test_export_configuration_servers_failure(self):
        ...
        with patch.object(
            orchestrator, "get_subscription_servers", return_value=mock_failed_result
        ):
            with pytest.raises(OrchestratorError) as exc_info:
...
def _create_mock_export_manager(routing_plugin=None, export_manager=None):
...
def _create_mock_export_manager_interface(client_profile=None):
