...
class TestLoadConfigFile:
    ...
    def test_load_yaml_file(self, tmp_path):
        ...
        config_data = {"logging": {"level": "INFO"}, "service": {"service_mode": True}}
...
class TestConfigFileValidation:
    ...
    def test_directory_as_config_file_validation(self, tmp_path):
        ...
        errors = exc_info.value.errors()
        assert any(
            "Configuration path is not a file" in str(error.get("msg", ""))
            for error in errors
        )
    ...
    def test_unreadable_config_file_validation(self, tmp_path):
        ...
        try:
            ...
            errors = exc_info.value.errors()
            assert any(
                "Configuration file is not readable" in str(error.get("msg", ""))
                for error in errors
            )
        finally:
