...
from src.sboxmgr.subscription.middleware import (
...
class TestMiddlewareIntegration:
    ...
    def test_route_config_middleware_basic(self):
        ...
        middleware = RouteConfigMiddleware(
            {"final_action": "direct", "override_mode": "profile_overrides"}
        )
    ...
    def test_middleware_with_profile_integration(self):
        ...
        profile = FullProfile(
            id="test-profile", metadata={"client_profile": client_profile.model_dump()}
        )
    ...
    def test_export_with_middleware_integration(self):
        ...
        # Create client profile
        client_profile = ClientProfile(
            routing={"final": "direct"}, exclude_outbounds=["vmess"]
        )
        ...
        config = singbox_export_with_middleware(
            servers, client_profile=client_profile, context=context
        )
    ...
    def test_middleware_chain_processing(self):
        ...
        outbound_filter = OutboundFilterMiddleware({"exclude_types": ["vmess"]})
        route_config = RouteConfigMiddleware({"final_action": "proxy-out"})
    ...
    def test_middleware_strict_mode(self):
        ...
        middleware = OutboundFilterMiddleware(
            {"exclude_types": ["vmess"], "strict_mode": True}
        )
    ...
    def test_middleware_override_modes(self):
        ...
        # Create client profile
        client_profile = ClientProfile(routing={"final": "block"})
        ...
        middleware_config = RouteConfigMiddleware(
            {"final_action": "direct", "override_mode": "config_overrides"}
        )
