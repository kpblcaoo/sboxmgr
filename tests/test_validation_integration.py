...
from sboxmgr.subscription.validators import GeoValidator, ProtocolSpecificValidator
...
def test_protocol_validator_with_real_data(
    ...
    if result.success and result.config:
        ...
        print(
            f"Protocol validation: {len(valid_servers)} valid, {len(invalid_servers)} invalid servers"
        )
        ...
        print(
            f"Could not test protocol validation - no servers retrieved: {result.errors}"
        )
...
@pytest.mark.integration
@pytest.mark.external
def test_required_fields_validator_with_real_data(
    test_subscription_url, require_external_tests
):
    ...
    if result.success and result.config:
        ...
        for server in result.config:
            try:
                ...
                invalid_servers.append(server)
                print(
                    f"Required fields validation error for server {server.address}: {e}"
                )

        print(
            f"Required fields validation: {len(valid_servers)} valid, {len(invalid_servers)} invalid servers"
        )
        ...
        print(
            f"Could not test required fields validation - no servers retrieved: {result.errors}"
        )


@pytest.mark.integration
@pytest.mark.external
def test_validation_pipeline_with_real_data(
    test_subscription_url, require_external_tests
):
    ...
    from sboxmgr.subscription.manager import SubscriptionManager
    from sboxmgr.subscription.models import SubscriptionSource
    from sboxmgr.subscription.validators import (
    ...
    if result.success and result.config:
        ...
        validators = [ProtocolSpecificValidator(), EnhancedRequiredFieldsValidator()]
        ...
        print(
            f"Validation pipeline: {valid_count} valid, {invalid_count} invalid servers"
        )
        ...
        print(
            f"Could not test validation pipeline - no servers retrieved: {result.errors}"
        )


@pytest.mark.integration
@pytest.mark.external
def test_parser_auto_detection_with_real_data(
    test_subscription_url, require_external_tests
):
    ...
    source_explicit = SubscriptionSource(
...
@pytest.mark.integration
@pytest.mark.external
def test_validation_error_details_with_real_data(
    test_subscription_url, require_external_tests
):
    ...
    if result.success and result.config:
        ...
        print(
            f"Could not test validation error details - no servers retrieved: {result.errors}"
        )
