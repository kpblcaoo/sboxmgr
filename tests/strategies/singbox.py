...
from hypothesis.strategies import builds, lists, text, integers, booleans, sampled_from, one_of
...
def unique_tag_strategy(prefix=""):
    ...
    return text(min_size=3, max_size=8, alphabet="abcdefghijklmnopqrstuvwxyz").map(lambda x: f"{prefix}{x}")
...
def tls_config_strategy():
    ...
    return builds(
        dict,
        enabled=booleans(),
        server_name=text(min_size=4, max_size=32, alphabet="abcdefghijklmnopqrstuvwxyz"),
        insecure=booleans(),
        alpn=lists(text(min_size=2, max_size=10, alphabet="abcdefghijklmnopqrstuvwxyz"), min_size=1, max_size=3)
    )
...
def singbox_config_strategy():
    ...
    return st.lists(
        unique_tag_strategy("out_"),
        min_size=1, max_size=5
    ).flatmap(lambda outbound_tags: builds(
        dict,
        log=builds(
            dict,
            level=sampled_from(["trace", "debug", "info", "warn", "error"])
        ),
        ...
        inbounds=st.one_of(
            ...
            st.lists(
                ...
                unique=True
            ).map(lambda indices: [inbound_strategy(f"in_{i}").example() for i in indices]),
        ...
        outbounds=st.one_of(
            ...
            st.lists(
                st.integers(min_value=0, max_value=len(outbound_tags)-1),
                min_size=1, max_size=len(outbound_tags),
                unique=True
            ).map(lambda indices: [outbound_strategy(outbound_tags[i]).example() for i in indices]),
        ...
        route=builds(
            ...
            rules=st.one_of(
                lists(
                    route_rule_strategy(outbound_tags),
                    min_size=0, max_size=3
                ),
                st.just([])  # Edge case: no rules
            ),
            final=sampled_from(outbound_tags) if outbound_tags else st.just("direct")
        )
    ))
...
def edge_case_configs():
    ...
    return st.one_of(
        ...
        st.just({
            "inbounds": [{"type": "http", "tag": "http", "listen": "127.0.0.1", "listen_port": 8080}]
        })
    ) 