...
from sboxmgr.config.validation import ConfigValidationError, validate_config_file
...
class TestGenerateConfig:
    ...
    def sample_template(self):
        ...
        return {
            ...
            "route": {
                "rules": [{"ip_cidr": "$excluded_servers", "outbound": "direct"}]
            },
        }
    ...
    def test_generate_config_template_not_found(self, tmp_path):
        ...
        with pytest.raises(FileNotFoundError, match="Template file not found"):
            generate_config(
                [], str(template_file), str(config_file), str(backup_file), []
            )
    ...
    def test_generate_config_no_change(
        self, tmp_path, sample_template, sample_outbounds
    ):
        ...
        with patch("sboxmgr.config.generate.info") as mock_info:
            ...
            mock_info.assert_called_with(
                "Configuration has not changed. Skipping update."
            )
    ...
    def test_generate_config_validation_failure(
        ...
        invalid_template = {
            "outbounds": [{"tag": "invalid"}],  # Missing required 'type' field
            "route": {
                "rules": [{"ip_cidr": "$excluded_servers", "outbound": "direct"}]
            },
        }
    ...
    def test_generate_config_internal_validation_success(
        self, tmp_path, sample_template, sample_outbounds
    ):
        ...
        result = generate_config(
            sample_outbounds, str(template_file), str(config_file), str(backup_file), []
        )
    ...
    def test_generate_config_config_dir_not_exists(
        self, tmp_path, sample_template, sample_outbounds
    ):
    ...
    def test_generate_config_with_backup(
        self, tmp_path, sample_template, sample_outbounds
    ):
