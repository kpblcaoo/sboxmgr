...
def test_export_manager_with_test_router():
    ...
    servers = [
        ParsedServer(
            type="ss", address="1.2.3.4", port=1234, security=None, meta={"tag": "test"}
        )
    ]
    exclusions = ["5.6.7.8"]
...
def test_default_router_with_servers_generates_tag():
    ...
    servers = [
        ParsedServer(type="vmess", address="5.6.7.8", port=443, security=None, meta={})
    ]
    routes = router.generate_routes(servers, [], [], context={"debug_level": 0})
...
def test_default_router_complete_rule_set():
    ...
    # Check exclusions (IP)
    assert any(
        "ip_cidr" in rule and "192.168.1.1/32" in rule["ip_cidr"] for rule in routes
    )
