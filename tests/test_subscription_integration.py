...
def test_subscription_pipeline_integration():
    ...
    try:
        ...
        source = SubscriptionSource(url=f"file://{temp_file}", source_type="url_base64")
        ...
        export_result = mgr.export_config(
            exclusions=[], user_routes=[], context=context, export_manager=export_mgr
        )
...
def test_subscription_with_exclusions():
    ...
    try:
        ...
        result = mgr.export_config(
            exclusions=exclusions, user_routes=[], context=context
        )
        ...
        proxy_outbounds = [
            o for o in outbounds if o.get("type") in ["ss", "vmess", "trojan", "vless"]
        ]
        if proxy_outbounds:
...
def test_subscription_pipeline_modes():
    ...
    try:
        source = SubscriptionSource(url=f"file://{temp_file}", source_type="url_json")
        ...
        assert result_tolerant.success or (
            result_tolerant.config is None or len(result_tolerant.config) == 0
        )
