...
def test_exclude_and_idempotent():
    ...
    result1 = run_cli(["--exclude", "1", "-u", "https://example.com/sub-link", "--dry-run"])
    # 2. Повторить добавление
    result2 = run_cli(["--exclude", "1", "-u", "https://example.com/sub-link", "--dry-run"])
...
def test_broken_exclusions_json():
    with open("exclusions.json", "w") as f:
        f.write("{broken json")
    result = run_cli(["--exclude", "2", "-u", "https://example.com/sub-link", "--dry-run"])
...
def test_view_exclusions():
    run_cli(["--exclude", "1", "-u", "https://example.com/sub-link", "--dry-run"])
    result = run_cli(["--exclude", "", "-u", "https://example.com/sub-link", "--dry-run"])
