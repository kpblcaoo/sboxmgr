...
class TestVersioning:
    ...
    def test_legacy_format_migration(self, tmp_path):
        ...
        pytest.skip(
            "DEPRECATED: Legacy format migration now handled by Pydantic model_validate()"
        )
...
class TestLoggingAndAudit:
    ...
    def test_add_exclusion_logging(self, tmp_path, caplog):
        ...
        # Check audit log
        assert (
            "Excluded server: Test Server [ID: server-123] (reason: Testing purposes)"
            in caplog.text
        )
    ...
    def test_remove_nonexistent_warning(self, tmp_path, caplog):
        ...
        assert (
            "Attempted to remove non-existent exclusion: nonexistent-123" in caplog.text
        )
...
class TestExclusionManagerEnhanced:
    ...
    def test_versioning_future_warning(self, temp_file, caplog):
        ...
        pytest.skip(
            "DEPRECATED: Future version warnings now handled by Pydantic validation"
        )
    ...
    def test_thread_safety_simulation(self, manager):
        ...
        def add_exclusions(start_id):
            for i in range(5):
                success = manager.add(
                    f"thread-{start_id}-{i}", f"Server {start_id}-{i}", "Thread test"
                )
    ...
    def test_set_servers_cache(self, manager, sample_json_data, supported_protocols):
        ...
        assert (
            len(manager._servers_cache["supported_servers"]) == 3
        )  # Excludes "direct"
    ...
    def test_list_servers(self, manager, sample_json_data, supported_protocols):
        ...
        assert all(
            len(info) == 3 for info in servers_info
        )  # (index, server, is_excluded)
        assert servers_info[0][0] == 0  # First server has index 0
        assert not servers_info[0][2]  # Not excluded initially

    def test_list_servers_with_exclusions(
        self, manager, sample_json_data, supported_protocols
    ):
    ...
    def test_format_server_info(self, manager):
        ...
        server = {"tag": "test-server", "type": "vmess", "server_port": 443}
    ...
    def test_add_by_index(self, manager, sample_json_data, supported_protocols):
        ...
        added_ids = manager.add_by_index(
            sample_json_data, [0, 1], supported_protocols, "Index test"
        )
    ...
    def test_add_by_wildcard(self, manager, sample_json_data, supported_protocols):
        ...
        added_ids = manager.add_by_wildcard(
            sample_json_data,
            ["server-*-1", "server-jp-*"],
            supported_protocols,
            "Wildcard test",
    ...
    def test_remove_by_index(self, manager, sample_json_data, supported_protocols):
        ...
        added_ids = manager.add_by_index(
            sample_json_data, [0, 1], supported_protocols, "Setup"
        )

        # Then remove by index
        removed_ids = manager.remove_by_index(
            sample_json_data, [0], supported_protocols
        )
        ...
        remaining_excluded = [
            server_id for server_id in added_ids if server_id not in removed_ids
        ]
    ...
    def test_invalid_index_handling(
        self, manager, sample_json_data, supported_protocols
    ):
        ...
        added_ids = manager.add_by_index(
            sample_json_data, [0, 999, -1], supported_protocols, "Invalid test"
        )

        # Should only add valid index (0)
        assert len(added_ids) == 1

    def test_empty_wildcard_pattern(
        self, manager, sample_json_data, supported_protocols
    ):
        ...
        added_ids = manager.add_by_wildcard(
            sample_json_data, ["nonexistent-*"], supported_protocols, "No match test"
        )
