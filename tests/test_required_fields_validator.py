...
def test_validator_accepts_valid_server():
    ...
    server = MockServer(type="vmess", port=443, address="example.com")
...
def test_validator_accepts_server_with_tag():
    ...
    server = MockServer(type="vmess", port=443, tag="valid-tag")
...
def test_validator_accepts_server_with_name():
    ...
    server = MockServer(type="vmess", port=443, meta={"name": "server-name"})
...
def test_validator_rejects_invalid_tag():
    ...
    server = MockServer(type="vmess", port=443, tag="invalid tag with spaces")
...
def test_validator_rejects_missing_identifier():
    ...
    server = MockServer(type="vmess", port=443)
...
def test_validator_uses_tag_as_address():
    ...
    server = MockServer(type="vmess", port=443, tag="my-server")
...
def test_validator_uses_name_as_address():
    ...
    server = MockServer(type="vmess", port=443, meta={"name": "my-server-name"})
