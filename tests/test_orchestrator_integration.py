...
def test_orchestrator_with_real_subscription(test_subscription_url, require_external_tests):
...
@pytest.mark.integration
@pytest.mark.external
def test_orchestrator_get_servers_with_real_subscription(test_subscription_url, require_external_tests):
    ...
    if result.success:
        ...
        if len(result.config) > 0:
            for server in result.config:
                ...
                assert server.protocol in ['vmess', 'vless', 'trojan', 'ss', 'shadowsocks']
    else:
        # If failed, check that we have meaningful error information
...
@pytest.mark.integration
@pytest.mark.external
def test_orchestrator_with_exclusions_and_real_subscription(test_subscription_url, require_external_tests):
    ...
    if result_without_exclusions.success and len(result_without_exclusions.config) > 0:
        ...
        if result_with_exclusions.success:
            assert len(result_with_exclusions.config) <= len(result_without_exclusions.config)
        ...
        print(f"Could not test exclusions - no servers retrieved: {result_without_exclusions.errors}")


@pytest.mark.integration
@pytest.mark.external
def test_orchestrator_export_different_formats(test_subscription_url, require_external_tests):
...
@pytest.mark.external
@pytest.mark.slow
def test_orchestrator_performance_with_real_data(test_subscription_url, require_external_tests):
...
@pytest.mark.integration
@pytest.mark.external
def test_orchestrator_with_different_source_types(test_subscription_url, require_external_tests):
    ...
    result_auto = orchestrator.get_subscription_servers(
        url=test_subscription_url
    )
