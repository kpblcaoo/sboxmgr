...
def test_cli_export_with_real_url(
    test_subscription_url, require_external_tests, tmp_path
):
...
@pytest.mark.integration
@pytest.mark.external
def test_cli_dry_run_with_real_url(
    test_subscription_url, require_external_tests, tmp_path
):
...
@pytest.mark.integration
@pytest.mark.external
def test_cli_list_servers_with_real_url(
    test_subscription_url, require_external_tests, tmp_path
):
...
@pytest.mark.integration
@pytest.mark.external
def test_cli_with_different_formats(
    test_subscription_url, require_external_tests, tmp_path
):
...
def test_cli_error_handling(test_subscription_url, require_external_tests, tmp_path):
    ...
    cmd_invalid = [
        ...
        "export",
        "--url",
        "https://invalid-url-that-does-not-exist.com/sub",
        "--output",
        "test_config.json",
    ]
    ...
    print(
        f"CLI error handling test completed with return code {result_invalid.returncode}"
    )
...
def test_cli_with_exclusions(test_subscription_url, require_external_tests, tmp_path):
    ...
    result_list = subprocess.run(list_cmd, check=False, capture_output=True, text=True, timeout=60)
    ...
    if result_list.returncode == 0:
        ...
        print(
            f"CLI exclusion test completed with return code {result_exclusion.returncode}"
        )
    else:
