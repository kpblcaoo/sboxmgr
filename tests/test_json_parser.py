...
def test_json_parser_debug_levels():
    ...
    with patch('sboxmgr.subscription.parsers.json_parser.get_debug_level', return_value=0):
        with pytest.raises(Exception):
            parser.parse(invalid_json)
    
    # Test debug_level = 1 (no output for > 1 check)
    with patch('sboxmgr.subscription.parsers.json_parser.get_debug_level', return_value=1):
        with pytest.raises(Exception):
            parser.parse(invalid_json)
    
    # Test debug_level = 2 (should trigger debug output)
    with patch('sboxmgr.subscription.parsers.json_parser.get_debug_level', return_value=2):
