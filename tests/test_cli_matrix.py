...
load_dotenv()
TEST_URL = (
    os.getenv("TEST_URL") or os.getenv("SINGBOX_URL") or "https://example.com/config"
)
...
CLI_MATRIX = [
    ...
    (
        ...
        "Dry-run: не должно быть изменений файлов",
        0,
        [],
        [
            "Dry run: config is valid",
    ...
    (
        [
            "export",
            "-u",
            "https://sub.vpn.momai.dev/GvFArer807ZY8vdg",
            "--output",
            "custom.json",
        ],
        "Кастомный output файл",
        0,
        ["custom.json"],
