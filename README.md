...
## Версия 1.3.1
- Полностью покрытый автотестами CLI (pytest)
- exclusions.json: SHA256 id + человекочитаемое имя
- clear-exclusions, идемпотентность, user-friendly ошибки
- Все основные сценарии CLI протестированы и защищены
- Wizard (установщик) — экспериментальная фича, не документируется в 1.3.1

## Тестирование
- Для запуска автотестов: `pytest -v tests/`
- Покрытие CLI: exclusions, clear, выбор, ошибки, dry-run, selected_config.json, --list-servers

## Быстрый старт
(оставить существующий quickstart, добавить инфо о exclusions и тестах)
